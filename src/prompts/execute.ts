...
3. **Write a report**: Before signaling completion or blocked, write a brief report of what you did:
   <task-report>
   Brief summary of work done, any issues encountered, and notable decisions made.
   </task-report>
4. **Signal completion**: When done, output the following XML tag on a separate line:
   <task-complete>DONE</task-complete>
5. **Signal if blocked**: If you cannot complete the task (missing dependencies, unclear requirements, external blocker), output:
   <task-blocked>Describe why you are blocked</task-blocked>
6. **Do not modify task statuses or tasks.json**: Never edit .taskmaster/tasks/tasks.json — do not change task statuses, add metadata, or modify any fields. The orchestrator manages the entire task lifecycle. Your only job is to signal via XML tags (<task-complete> or <task-blocked>).
