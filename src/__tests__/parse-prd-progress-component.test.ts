/**
 * Structural tests for ParsePrdProgress.vue — verifies component structure,
 * data-testid attributes, conditional rendering, and wiring to logic helpers.
 *
 * Uses source-level assertions (same approach as agent-chat-panel-input.test.ts)
...
const componentPath = resolve(__dirname, "../../ui/src/components/ParsePrdProgress.vue");
...
describe("ParsePrdProgress component structure", () => {
  describe("imports", () => {
    it("imports ParsePrdMessage, ParsePrdStoreState, ParsePrdOutcome types", () => {
      expect(scriptSection).toContain("ParsePrdMessage");
      expect(scriptSection).toContain("ParsePrdStoreState");
      expect(scriptSection).toContain("ParsePrdOutcome");
    });

    it("imports pure logic helpers from parse-prd-progress-logic", () => {
      expect(scriptSection).toContain("parse-prd-progress-logic");
  ...
  describe("template: data-testid attributes", () => {
    it("has parse-prd-panel on root element", () => {
      expect(templateSection).toContain('data-testid="parse-prd-panel"');
    });

    it("has parse-prd-status-text on status label", () => {
      expect(templateSection).toContain('data-testid="parse-prd-status-text"');
    });

    it("has parse-prd-stop-button on stop button", () => {
      expect(templateSection).toContain('data-testid="parse-prd-stop-button"');
    });

    it("has parse-prd-outcome-banner on outcome banner", () => {
      expect(templateSection).toContain('data-testid="parse-prd-outcome-banner"');
    });

    it("has parse-prd-outcome-label on outcome label", () => {
      expect(templateSection).toContain('data-testid="parse-prd-outcome-label"');
    });

    it("has parse-prd-outcome-errors on error list", () => {
      expect(templateSection).toContain('data-testid="parse-prd-outcome-errors"');
    });

    it("has parse-prd-dismiss-button on dismiss button", () => {
      expect(templateSection).toContain('data-testid="parse-prd-dismiss-button"');
    });
  });
  ...
  describe("template: header section", () => {
    ...
    it("renders stop button with severity danger", () => {
      // Check the Button has severity="danger"
  ...
  describe("template: message output section", () => {
    ...
    it("renders error messages with error styling", () => {
      ...
      expect(templateSection).toContain("pprd-line--error");
    });
  ...
  describe("styles", () => {
    ...
    it("defines root .pprd-progress class", () => {
      expect(styleSection).toContain(".pprd-progress");
    });

    it("defines header class", () => {
      expect(styleSection).toContain(".pprd-progress__header");
    });

    it("defines status class", () => {
      expect(styleSection).toContain(".pprd-progress__status");
    });

    it("defines output area class", () => {
      expect(styleSection).toContain(".pprd-progress__output");
    });

    it("defines dot variant classes for all states", () => {
      expect(styleSection).toContain(".pprd-dot--active");
      expect(styleSection).toContain(".pprd-dot--stopping");
      expect(styleSection).toContain(".pprd-dot--completed-success");
      expect(styleSection).toContain(".pprd-dot--completed-failure");
      expect(styleSection).toContain(".pprd-dot--completed-cancelled");
    });

    it("defines banner severity classes", () => {
      expect(styleSection).toContain(".pprd-banner--success");
      expect(styleSection).toContain(".pprd-banner--error");
      expect(styleSection).toContain(".pprd-banner--warning");
    });

    it("defines pulse animation", () => {
      expect(styleSection).toContain("pprd-pulse");
    });

    it("defines dismiss button styling", () => {
      expect(styleSection).toContain(".pprd-banner__dismiss");
    });
